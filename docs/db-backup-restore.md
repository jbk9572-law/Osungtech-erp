...
환경설정 > 백업/복원 > **"재해복구(서버에서 시점 복원)"** 화면에서 관리자가
바로 실행할 수 있다. 시점을 고르고 확인 코드를 입력해 제출하면, 앱은
`.github/workflows/db-restore.yml`을 GitHub API로 `workflow_dispatch`
호출만 한다 — 실제 `pg_restore --clean`은 GitHub Actions 러너에서 실행되고,
**항상 `SUPABASE_DB_URL`이 가리키는 그 DB(=백업을 뜨는 그 운영 DB)에
그대로 적용된다.** 즉 이 버튼은 "다른 프로젝트에 복사해서 확인해보기"용이
아니라, 진짜로 지금 운영 DB를 그 시점으로 되돌리는 기능이다.

> 원래는 이 위험 때문에 앱 안에 복원 버튼을 두지 않기로 했었다. 이후
> "그래도 버튼은 만들되 접근을 엄격하게 관리하자"는 방향으로 다시
> 정했고, 그래서 아래처럼 여러 겹의 안전장치를 뒀다:
> - 관리자만 화면 자체에 접근 가능(`requireAdmin`).
> - 시점을 고를 때마다 새로 뽑는 4자리 확인 코드를 정확히 입력해야
>   제출 버튼이 활성화된다(다른 시점으로 바꿔 골랐는데 예전 코드를
>   그대로 치는 실수 방지).
> - 워크플로우 쪽에서도 `confirm` 입력값이 정확히 `RESTORE`가 아니면
>   즉시 실패한다 — 앱을 거치지 않고 Actions 탭에서 직접
>   "Run workflow"를 눌러도 이 문구 없이는 절대 실행되지 않는다.
> - 이 기능 자체가 `GITHUB_RESTORE_TOKEN` 환경변수가 없으면 꺼져 있다
>   (아래 설정 참고) — 필요 없는 배포본은 그냥 안 만들면 된다.

**설정 (1회만)**: GitHub **Settings > Developer settings > Fine-grained
personal access tokens**에서 이 저장소(`Osungtech-erp`) 하나에만 접근하는
토큰을 새로 만든다. 권한은 딱 둘만 준다 — **Actions: Read and write**
(워크플로우를 실행시키는 데 필요), **Contents: Read-only**(백업 시점
목록을 조회하는 데 필요). 발급받은 토큰을 배포 환경(Netlify라면
Site settings > Environment variables)에 `GITHUB_RESTORE_TOKEN`이라는
이름으로 등록한다. 그 외 값은 필요 없다 — 저장소/워크플로우 이름은
코드(`src/lib/github-restore.ts`)에 이미 고정돼 있다.

### 터미널에서 복원하기 (다른 프로젝트로 복사할 때)

재해복구 훈련처럼 **운영 DB가 아닌 다른 Supabase 프로젝트**에 복원해보고
싶을 때는 여전히 이 방법을 쓴다 — 위 앱 버튼은 항상 운영 DB만 대상으로
하기 때문에 다른 프로젝트로는 못 보낸다.
...
기존 테이블/함수가 충돌하면 지우고 다시 만든다. 아래 두 경우에 쓴다.

- **재해복구 훈련**: 무료로 새 Supabase 프로젝트를 하나 더 만들고(2분,
  카드 등록 불필요), 그 프로젝트의 pooling 연결 문자열을 대상으로 복원해
  실제로 되살아나는지 주기적으로 확인해본다. 훈련이 끝나면 그 프로젝트는
  지워도 된다.
- **운영 DB 자체를 새 프로젝트로 옮겨야 하는 경우**: 기존 프로젝트가
  완전히 망가져 새 Supabase 프로젝트를 새로 만들어야 할 때, 그 새
  프로젝트에 최신 백업을 복원한다(이 경우 `SUPABASE_DB_URL` /
  `GITHUB_RESTORE_TOKEN`이 가리키는 대상도 새 프로젝트로 다시
  등록해야 위 앱 버튼이 계속 그 DB를 향하게 된다).
